"""
fit_is.py - 1D Inception Score (IS) Evaluation
"""
import os
import random
import numpy as np
...
SIMULATED_DATA_DIR = r'E.\simulated_dataset'
# ==============================================================
...
def calculate_is(features_sim, gmm_model):
    eps = 1e-12
    p_yx = gmm_model.predict_proba(features_sim)
    p_yx = np.clip(p_yx, eps, 1.0)
    ...
    for i in range(p_yx.shape[0]):
        kl = np.sum(p_yx[i] * np.log(p_yx[i]) - p_yx[i] * np.log(p_y))
        kl_divs.append(kl)
...
if __name__ == '__main__':
    ...
    # 3. Train a GMM as a mock classifier for IS
    print(">>> Training Gaussian Mixture Model (GMM) as a mock classifier...")
    gmm = GaussianMixture(n_components=8, covariance_type='diag', random_state=2026)
    gmm.fit(feat_real)

    # 4. Define Simulated Datasets Paths
    datasets_paths = {}
    for i in range(1, 13):
    ...
    for cat, path in datasets_paths.items():
        if os.path.exists(path):
            ...
            score = calculate_is(feat_sim, gmm)
            print(f"[{cat}] IS Score: {score:.16f}")
        else:
